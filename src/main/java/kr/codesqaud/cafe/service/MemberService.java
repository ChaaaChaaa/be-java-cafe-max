...
public class MemberService {
	...
	public MemberResponseDto findById(Long memberId) {
		return MemberResponseDto.of(
			memberRepository.findById(memberId).orElseThrow(() -> new CommonException(CommonExceptionType.NOT_FOUND)));
	}

	@Transactional(readOnly = true)
	public MemberResponseDto findByEmail(String email) {
		return MemberResponseDto.of(
			memberRepository.findByEmail(email).orElseThrow(() -> new CommonException(CommonExceptionType.NOT_FOUND)));
	}

	@Transactional
	public void update(ProfileEditRequestDto profileEditRequestDto) {
		Member findMember = memberRepository.findById(profileEditRequestDto.getMemberId())
			.orElseThrow(() -> new CommonException(CommonExceptionType.NOT_FOUND));
		...
		if (findMember.isDifferentNickname(profileEditRequestDto.getNickname())) {
			memberRepository.findByNickname(profileEditRequestDto.getNickname())
				.ifPresent(m -> {
					throw new MemberProfileEditException(MemberExceptionType.DUPLICATED_MEMBER_NICKNAME,
	...
	public LoginMemberSession login(MemberLoginRequestDto memberLoginRequestDto) {
		Member member = memberRepository.findByEmail(memberLoginRequestDto.getEmail())
			.orElseThrow(() -> new MemberLoginException(MemberExceptionType.NOT_FOUND, memberLoginRequestDto));
